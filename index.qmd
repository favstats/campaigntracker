...
*Im KI Explorer* können alle erkannten KI-generierten Inhalte der Parteien angezeigt und nach bestimmten Kriterien sortiert werden. Die Beiträge lassen sich nach *Partei, Beitragstext, Plattform, Post/Anzeige* filtern, um spezifische Muster und Trends sichtbar zu machen.
...
labeled <- readRDS("data/labeled.rds")  %>% 
  mutate(party = case_when(
    ...
    party %in% c("MLPD – Marxistisch-Leninistische Partei Dxxxeutschlands") ~ "MLPD",
...
allmeds <- dir("organic", recursive = TRUE, all.files = TRUE, full.names = T) %>% 
  c(dir("/mnt/storage/organic", recursive = TRUE, all.files = TRUE, full.names = T)) %>% 
  c(dir("/mnt/storage/paid", recursive = TRUE, all.files = TRUE, full.names = T)) %>% 
...
# 
# result %>%
#   distinct(ai_hash, .keep_all = T) %>% View()
# 
# allmeds %>% keep(~str_detect(.x, "frames"))
