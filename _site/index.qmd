...
```{r}
# setwd("/home/favstats/campaigntracker")
...
full_data <- readRDS("../data/igfb.rds") %>% mutate_at(vars(ai_generated, ai), as.numeric)
workwith <- full_data
# workwith %>% 
#   count(ai)
...
topic_data <- full_data %>%
  ...
  select(get_text("topic_other_political", lang):get_text("topic_general_election", lang)) %>% 
...
theme_bild_video_data <- full_data %>%
  ...
  select(get_text("theme_event_announcement", lang):get_text("theme_other_image_content", lang)) %>% 
...
```
```{r, eval = F}
# Compute median in the original scale
median_values <- full_data %>% 
  select(likes, ai) %>% 
  mutate(likes = as.numeric(likes)) %>% 
  mutate(ai = as.character(ai)) %>%
  group_by(ai) %>%
  summarise(median_likes = median(likes, na.rm = TRUE))

full_data %>% 
  filter(type == "organic") %>% 
  select(likes, ai) %>% 
  # count(likes, sort = T) %>% View()
  # filter(is.na(likes)) %>% 
  mutate(likes = as.numeric(likes)) %>% 
  mutate(ai = as.character(ai)) %>% 
  ggplot(aes(x = ai, y = likes)) +
  geom_boxplot(aes(fill = ai), alpha = 0.6) +  # Boxplot with color
  scale_y_log10() +  # Log scale
  geom_text(data = median_values, aes(x = ai, y = median_likes, 
            label = paste0("Median = ", round(median_likes, 1))), 
            vjust = -0.5, fontface = "bold", size = 5, color = "black") +  # Correct median display
  labs(
    title = "Distribution of Likes by AI (Log Scale)",
    x = "AI Model",
    y = "Likes (Log Scaled)"
  ) +
...
only_ai_parties <- workwith %>% filter(ai == 1) %>% pull(party)

full_data %>% 
  select(likes, ai, party) %>% 
  mutate(likes = as.numeric(likes)) %>% 
  mutate(ai = as.character(ai)) %>% 
  filter(party %in% only_ai_parties) %>% 
  mutate(party = fct_reorder(party, likes)) %>% 
  ggplot(aes(x = party, y = likes)) +
  geom_boxplot(aes(fill = ai), alpha = 0.6) +  # Boxplot with color
  scale_y_log10() +  # Log scale
  # geom_text(data = median_values, aes(x = ai, y = median_likes, 
  #           label = paste0("Median = ", round(median_likes, 1))), 
...
organic_b <- readRDS("~/campaigntracker/data/organic.rds") %>% bind_rows(readRDS("~/campaigntracker/data/organic_ig.rds"))

missing_likes <- full_data %>% 
  filter(type == "organic") %>% 
  filter(is.na(likes)) %>% 
  select(likes, platform, everything())

add_likes <- organic_b %>% 
  filter(url %in% missing_likes$url_eintragen_url) %>% 
  select(url, likes_o = likes)

# Datenvorbereitung: likes als numerisch; nur positive Werte; KI-Label in Deutsch; Log-Transformation
data_prepped <- full_data %>% 
  left_join(add_likes) %>% 
  mutate(likes = ifelse(is.na(likes), likes_o, likes)) %>% 
  # bind_rows(add_likes, .) %>% 
  ...
  # filter(likes >= 0) %>%  # ausschließen, da log(0) nicht definiert
  mutate(likes = ifelse(likes == 0, 1, likes)) %>% 
  mutate(
    ai = ifelse(ai == 1, "Mit KI", "Ohne KI"),
    # party = fct_reorder(party, likes),
...
# organic
 ...
 #  filter(type == "organic") %>% 
 #  filter(is.na(likes)) %>% 
 #  select(likes, platform, everything()) %>% View()
 #   count(platform)
 # 
 #  organic_b %>% 
 #    filter(str_detect(url, "DGJVSwBukzA")) %>% View()
```


```{r}
# Berechne die Median-Werte für die Annotationen
medians <- data_prepped %>%
  group_by(ai) %>%
  summarise(
    median_value = median(log_likes, na.rm = TRUE),
    total_value = sum(likes, na.rm = TRUE),
    n_value = n()
  ) %>% 
  mutate(median_original = round(10^median_value))  # zurücktransformiert
```



```{r}
# Median-Labels vorbereiten
annotations <- list(
  list(
    x = "Mit KI",  # Kategorie auf der x-Achse
    y = medians$median_value[medians$ai == "Mit KI"],  # Median-Wert (log-skaliert)
    label = list(
      text = paste("Median:", format(medians$median_original[medians$ai == "Mit KI"], big.mark = ".", decimal.mark = ","), "Likes"),
      style = list(
        color = "#FF6F61",
        fontSize = "12px",
        fontWeight = "bold"
  ...
  list(
    x = "Ohne KI",
    y = medians$median_value[medians$ai == "Ohne KI"],
    label = list(
      text = paste("Median:", format(medians$median_original[medians$ai == "Ohne KI"], big.mark = ".", decimal.mark = ","), "Likes"),
      style = list(
        color = "#6B5B95",
        fontSize = "12px",
        fontWeight = "bold"
 ...
 max_likes <- max(as.numeric(data_prepped$likes), na.rm = TRUE)

# Boxplot mit logarithmischer y-Achse (transformierte Daten) und deutscher Beschriftung
boxies <- apex(data_prepped, aes(x = ai, y = log_likes, color = ai, group = ai), "boxplot") %>%
  ax_plotOptions(
    boxPlot = boxplot_opts(color.upper = "#FF6F61", color.lower = "#8BB0A6")
  ) %>%
  # Manually assign different colors per group
  ...
  # ) %>%
  ax_yaxis(
    title = list(text = "Gefällt mir (logarithmisch)"),
    labels = list(formatter = htmlwidgets::JS("function(val){ return Math.pow(10, val).toFixed(0); }"))
  ) %>%
  # ax_xaxis(
  #   title = list(text = "")
  # ) %>%
  ax_title(text = "Verteilung Likes von Beiträgen mit und ohne KI")
boxies <- apex(data_prepped, aes(x = ai, y = log_likes, color = ai, group = ai), "boxplot") %>%
  ax_plotOptions(
  ...
  ax_yaxis(
    title = list(text = "Likes (logarithmisch)"),
    labels = list(
      formatter = htmlwidgets::JS("
        function(val) {
          return Math.pow(10, val).toFixed(0);  // Normale Zahlenanzeige
        }
      ")
    ),
    min = -1#,  # Minimum der Y-Achse
    # max = log10(max_likes)  # Maximale Y-Achse auf das tatsächliche Maximum setzen
  )  %>%
  ax_tooltip(
    shared = FALSE,
    custom = htmlwidgets::JS("
      function({ seriesIndex, dataPointIndex, w }) {
        var thePoint = w.config.series[seriesIndex].data[dataPointIndex];
        var label = thePoint.x;

        var arr = thePoint.y;  // [Min, Q1, Median, Q3, Max]

        // Zurückrechnen in Originalwerte
        var minVal    = Math.pow(10, arr[0]).toFixed(0);
        var q1Val     = Math.pow(10, arr[1]).toFixed(0);
        var medianVal = Math.pow(10, arr[2]).toFixed(0);
        var q3Val     = Math.pow(10, arr[3]).toFixed(0);

        // Tooltip ohne den irreführenden Maximalwert
        return `
          <div style='padding: 8px; font-size: 14px;'>
            <strong>${label}</strong><br/>
            <hr style='margin: 4px 0;'>
            Min: ${minVal} Likes<br/>
            1. Quartil (25%): ${q1Val} Likes<br/>
            Median: ${medianVal} Likes<br/>
            3. Quartil (75%): ${q3Val} Likes<br/>
            <br/>
            <em>Die Hälfte der Beiträge hat mehr als ${medianVal} Likes erhalten.</em>
          </div>
        `;
      }
    ")
  )%>%
  ax_xaxis(title = list(text = "")) %>%
  ax_title(text = "Verteilung Likes von Beiträgen mit und ohne KI") %>%
  ax_legend(
    labels = list(
      formatter = htmlwidgets::JS("
        function(seriesName) {
          return seriesName === 'Mit KI'
            ? 'Beiträge mit KI'
            : 'Beiträge ohne KI';
        }
      ")
  ...
  ax_annotations(
    points = annotations  # Hier werden die Mediane als sichtbare Punkte angezeigt
  ) 
...
#   mutate(likes = as.numeric(likes)) %>%
#   arrange(desc(likes)) %>%
#   select(page_name, text, likes)
```


<br>

Die Analyse umfasst **insgesamt `r scales::comma(sum(medians$n_value), accuracy = 1)` Beiträge**, die entweder **mit** oder **ohne** den Einsatz von Künstlicher Intelligenz (KI) erstellt wurden. Davon stammen **`r medians$n_value[medians$ai == "Mit KI"]` Beiträge aus der Kategorie mit KI** und **`r scales::label_number(scale_cut = scales::cut_short_scale())(medians$n_value[medians$ai == "Ohne KI"])`** aus der Kategorie **ohne KI**. 

Die Beiträge **mit KI** erzielten insgesamt **`r scales::label_number(scale_cut = scales::cut_short_scale())(medians$total_value[medians$ai == "Mit KI"])` Likes**, während die Beiträge **ohne KI `r scales::label_number(scale_cut = scales::cut_short_scale())(medians$total_value[medians$ai == "Ohne KI"])` Likes** erreichten.

Beim **Median der Likes** – also die Anzahl der Likes, die ein typischer Beitrag in der Mitte der Verteilung erhält – zeigt sich dass Beiträge **mit KI** im Median **`r medians$median_original[medians$ai == "Mit KI"]` Likes erhielten**, während Beiträge **ohne KI `r medians$median_original[medians$ai == "Ohne KI"]` Likes** erzielten. 
...
# Filter parties that used AI
only_ai_parties <- workwith %>% filter(ai == 1) %>% pull(party)

# Convert party_colors to a named vector
party_colors_vector <- unlist(party_colors)
...
# Calculate median likes per party for annotation
medians <- data_prepped %>%
  group_by(party, ai) %>%
  summarise(
    median_value = median(log_likes, na.rm = TRUE),
  ...
  mutate(median_original = round(10^median_value))  # Back-transform

# Prepare function to create each chart
create_party_chart <- function(party_name) {
  party_data <- data_prepped %>% filter(party == party_name)
  party_color <- party_colors_vector[party_name] %>% unname()
  party_median <- medians %>% filter(party == party_name)

annotations <- list(
  list(
    x = "Mit KI",  # Kategorie auf der x-Achse
    y = party_median$median_value[party_median$ai == "Mit KI"],  # Median-Wert (log-skaliert)
    label = list(
      text = paste("Median:", format(party_median$median_original[party_median$ai == "Mit KI"], big.mark = ".", decimal.mark = ","), "Likes"),
      style = list(
        color = "#FF6F61",
        fontSize = "12px",
        fontWeight = "bold"
  ...
  list(
    x = "Ohne KI",
    y = party_median$median_value[party_median$ai == "Ohne KI"],
    label = list(
      text = paste("Median:", format(party_median$median_original[party_median$ai == "Ohne KI"], big.mark = ".", decimal.mark = ","), "Likes"),
      style = list(
        color = "#6B5B95",
        fontSize = "12px",
        fontWeight = "bold"
  ...
  max_likes <- max(party_data$likes, na.rm = TRUE)
# print(party_data)
party_data <<- party_data
  # Create chart
  chartss <- apex(party_data, aes(x = ai, y = log_likes), "boxplot") %>%
    ax_plotOptions(
      boxPlot = boxplot_opts(
        color.upper = party_color,
        color.lower = party_color
      )
    ) %>%
    ax_yaxis(
      title = list(text = "Likes (logarithmisch)"),
      labels = list(
        formatter = htmlwidgets::JS("
          function(val) {
            return Math.pow(10, val).toFixed(0);
          }
        ")
    ),
    min = -1#,  # Minimum der Y-Achse
    ...
    ax_title(text = paste("Verteilung der Likes -", party_name)) %>%
    ax_annotations(points = annotations) %>%
    ax_chart(
      toolbar = list(show = FALSE),
      animations = list(enabled = TRUE)
    )
...
## Anzahl an Likes für Beiträge mit und ohne KI nach Partei (nur Posts)
...
htmltools::tagList(
  ...
  tags$div(
    class = "apex-container",
htmltools::browsable(
create_party_chart("Freie Wähler"))
  )
)
```
...
htmltools::tagList(
  ...
  tags$div(
    class = "apex-container",
htmltools::browsable(
create_party_chart("Piratenpartei"))
  )
)
```
...
htmltools::tagList(
  ...
  tags$div(
    class = "apex-container",
htmltools::browsable(
create_party_chart("Bayernpartei"))
  )
)
```
...
htmltools::tagList(
  ...
  tags$div(
    class = "apex-container",
htmltools::browsable(
create_party_chart("NPD /Die Heimat"))
  )
)
```
...
full_data %>% 
  ...
  mutate(party = fct_reorder(party, likes)) %>% 
  arrange(party) %>% 
  # filter(party == "CDU/CSU") %>% View()
  ggplot(aes(x = party, y = likes)) +
  geom_boxplot() +  # Boxplot with color
  scale_y_log10() +  # Log scale
  # geom_text(data = median_values, aes(x = ai, y = median_likes, 
  #           label = paste0("Median = ", round(median_likes, 1))), 
  #           vjust = -0.5, fontface = "bold", size = 5, color = "black") +  # Correct median display
